...
19. [What Was Actually Verified On This GPU, and What Still Needs Work](#19-what-was-actually-verified-on-this-gpu-and-what-still-needs-work)
...
| Backbone | Mostly frozen — only the last 2 transformer blocks + final LayerNorm are trainable |
| Trainable part | Last 2 backbone blocks + classification head (3,650,823 params) |
| Head architecture | LayerNorm → Linear(384→256) → GELU → Dropout(0.3) → Linear(256→7) |
| Loss function | CrossEntropyLoss with inverse-frequency class weights (`--class_weights`) |
...
### Why the Backbone Is Mostly Frozen

DINO was pretrained on 1.28 million images. RAF-DB has only 12,271 training images — about 100 times fewer. Fine-tuning all 21 million backbone parameters on this small dataset would cause severe overfitting: the model would memorise the training faces rather than learn general emotion patterns. By freezing most of the backbone and only training the last 2 transformer blocks plus a small head, we get strong generalisation to faces the model has never seen before while still letting the top of the network adapt slightly to emotion-specific features.

**Tested alternative — class-weighted loss:** RAF-DB is heavily imbalanced (Happy is 38.9% of the training set, Fear is only 2.3%). Training with plain CrossEntropyLoss lets the model ignore rare classes with little penalty. Adding inverse-frequency class weights (so Fear is weighted ~3× and Happy ~0.18×) traded 0.4 points of overall accuracy for a large, broad-based improvement: macro F1 rose from 69.2% to 70.8%, and the worst-performing class (Disgust) improved from 40.6% to 61.9% recall. This is the configuration now used for the official checkpoint — see [Section 19](#19-what-was-actually-verified-on-this-gpu-and-what-still-needs-work) for the full before/after comparison.

### Results

*(Measured on an NVIDIA RTX A3000 12GB Laptop GPU — see Section 19 for how these were obtained and for the frozen-backbone/unweighted-loss baseline this improved on.)*

| Metric | Value |
|--------|-------|
| Test Accuracy | **78.36%** |
| Macro F1-Score | **70.78%** |
| Best class | Surprise (88.1% recall) |
| Hardest class | Fear (54.1% recall) |
...
*(Measured on an NVIDIA RTX A3000 12GB Laptop GPU.)*

| Metric | Value |
|--------|-------|
| Validation Accuracy | **42.36%** |
| Macro F1 | **34.94%** |
| Notes | RAVDESS has only 1,440 samples. Audio-only recognition is harder than face-based recognition, and the dataset is very small. 42.36% is well above random chance (14.3% for 7 classes). |

**Tested alternative — unfreezing the backbone:** fully fine-tuning all 95M Wav2Vec2 parameters (`--unfreeze`, lr=5e-5, batch_size=8) for 15 epochs was also tried, hypothesising it might close the gap the way it can for larger datasets. It performed *worse* — 28.47% validation accuracy, still near-random after 8 epochs — because 1,296 training clips is nowhere near enough to safely update 95M parameters in that time budget. This confirms the frozen-backbone design decision (Section 15, Decision 2) was correct for this dataset size, at least without a much longer training budget or a warmup/lower-LR schedule. See [Section 19](#19-what-was-actually-verified-on-this-gpu-and-what-still-needs-work) for the full log.
...
| Metric | Value |
|--------|-------|
| Validation Accuracy | **93.51%** (initial run) / **89.90%** (after retraining against the class-weighted face checkpoint) |
| Notes | High accuracy is expected on synthetic same-class pairs — this is an upper bound, not a measure of real cross-modal alignment. The real-world test is the qualitative demo on live video input. The drop from 93.51% to 89.90% happened because the face checkpoint's embeddings shifted when class-weighted loss was adopted (Section 6); fusion was retrained on the new embeddings but the two effects partially offset. |
...
|-------|-------|---------|----------|-------|
| Phase 1 | DINO ViT-S/16 | RAF-DB (test set) | **78.36%** | 70.78% macro F1, class-weighted loss |
| Phase 2 | Wav2Vec2-base-960h | RAVDESS (validation) | **42.36%** | Small dataset; backbone frozen |
| Phase 3 | Late-Fusion MLP | Synthetic pairs | **89.90%** | 4,000 same-class synthetic pairs |

*Measured on an NVIDIA RTX A3000 12GB Laptop GPU — see [Section 19](#19-what-was-actually-verified-on-this-gpu-and-what-still-needs-work) for how these numbers were obtained, what was tried and rejected, and the gap between these and earlier unverified figures.*
...
| `audio_data_cremad/` | CREMA-D WAV files (flat folder, 7,442 clips) — tested as an audio dataset expansion, not currently used by the official checkpoint; see Section 19.3/19.4 |
...
**Locally (with a GPU, ~20 min on an RTX A3000 12GB laptop GPU; hours on CPU):**
```powershell
python src\train.py --epochs 20 --batch_size 32 --lr 1e-4 --class_weights --num_workers 0
```
`--class_weights` is recommended (see Section 19.3) — it trades a fraction of a point of overall accuracy for a meaningfully more balanced model across classes. `--num_workers 0` avoids a DataLoader hang seen on Windows with `pin_memory=True` and workers > 0. Measured result: 78.36% test accuracy / 70.78% macro F1 after 20 epochs (Section 19.2).
...
## 19. What Was Actually Verified On This GPU, and What Still Needs Work

### 19.1 The problem this section fixes

Every result number in this README used to be unverifiable: `checkpoints/` and `results_*.json` were git-ignored, no training run had ever actually finished and logged a real result, and the charts in `images/` were mathematically smooth curves with zero epoch-to-epoch noise — a tell that they were plotted from a formula, not real training. Three bugs explain why:

1. **`src/train.py` crashed before saving results.** The per-class accuracy printout used a Unicode block character (`█`) that Windows' default `cp1252` console codepage cannot encode. The crash happened *after* the checkpoint was saved but *before* `results_dino.json` was written, so every previous run produced a checkpoint with no accompanying log — and `generate_results_visuals.py` would then silently fall back to old hardcoded numbers. Fixed by switching to `#`.
2. **The documented NaN fix for audio training was missing from the code.** README Section 7 described overriding `AudioFERModel.train()` to keep the Wav2Vec2 backbone in `eval()` mode (preventing SpecAugment from producing NaN losses), but that override didn't actually exist in `src/audio_model.py`. Added it back.
3. **`generate_results_visuals.py`'s terminal-summary panels had the old numbers hardcoded as literal strings**, separate from the (correctly real) training-curve plots. Even a from-scratch retrain would have regenerated charts with stale text. Rewritten to read every number from `results_*.json`.

### 19.2 Real results, trained end-to-end on an NVIDIA RTX A3000 12GB Laptop GPU

| Phase | Metric | Previously claimed (unverified) | Actually measured |
|---|---|---|---|
| 1 — Face (DINO), frozen backbone + unweighted loss | Test accuracy | 80.67% | **78.72%** |
| 1 — Face (DINO), **class-weighted loss (adopted)** | Test accuracy / macro F1 | — | **78.36% / 70.78%** |
| 2 — Audio (Wav2Vec2), frozen backbone | Val accuracy / macro F1 | 43.1% | **42.36% / 34.94%** |
| 3 — Fusion, against class-weighted face checkpoint | Val accuracy | 95.65% | **89.90%** |

The previously claimed numbers turned out to be close to reality — the original methodology was sound, it just never finished a run that proved it. See `images/results_comparison_old_vs_new.png` for the chart and `checkpoints/results_*.json` for the full per-epoch history behind every number above.

**Confusion matrix and trivial baselines** (`images/face_confusion_matrix.png`, `images/face_baseline_comparison.png`): the face model (78.36%) clears a majority-class baseline of always predicting "Happy" (38.62%, computed directly from the test set — this number appears in the original Colab notebook's final-report code but was never actually run) by 40 points, and random chance (14.3%) by 64 points, so the accuracy reflects real signal rather than exploiting class imbalance. The confusion matrix shows interpretable errors, not noise: Fear is most often predicted as Surprise (22% of Fear images), and Disgust as Neutral (16%) — both well-documented visual look-alikes in facial emotion recognition.

### 19.3 Improvement experiments tried

Two changes were tested against the frozen-backbone-only baselines described in Sections 6–7. Both were run to completion, not just theorised:

| Experiment | Result | Adopted? |
|---|---|---|
| **Class-weighted loss for the face model** (`--class_weights`, inverse-frequency weighting — RAF-DB is 38.9% Happy vs 2.3% Fear) | Macro F1 69.23% → **70.78%**. Disgust recall 40.6% → **61.9%** (the weakest class, dramatically improved). Overall accuracy cost: −0.36 points. Happy/Neutral recall dropped a few points as a direct trade-off. | **Yes** — this is now the official `checkpoints/best_dino.pt`. |
| **Fully unfreezing the Wav2Vec2 backbone** (`--unfreeze`, lr=5e-5, batch 8, 15 epochs) | Val accuracy 42.36% → **28.47%**, worse. Model was still near-random (~20%) for the first 8 of 15 epochs — 1,296 training clips isn't enough to safely update 95M parameters in that time budget. | No — kept the frozen-backbone checkpoint. Logged in `experiments/checkpoints_audio_unfrozen/` for reference. |
| **Adding CREMA-D** (7,442 clips, downloaded via `git`/`git-lfs`, combined with RAVDESS via new `SpeechEmotionDataset.combine()`) | Evaluated fairly on the *same* RAVDESS validation split as the baseline: 42.36% → **35.42%**, worse, despite 6.2x more training data. Likely domain shift between the two corpora (different actors/mics/scripts). | No — kept the RAVDESS-only checkpoint. Logged in `experiments/checkpoints_audio_combined/`. Data and combining code both kept — see §19.4 for a better way to use them. |
| **Optuna hyperparameter search** for the face model (`src/tune_optuna.py`, previously unused — fixed to use class-weighted loss to match the deployed config; 15 trials × 4 epochs, then the winning trial trained to a full 20 epochs) | Best trial reached 74.4% after only 4 epochs (lr=2.4e-4, unfreeze last 4 blocks) vs. the official config's slower start — but trained to the full 20 epochs it scored 78.25% / macro F1 69.59%, **not better than** the official 78.36% / 70.78%. A textbook short-budget-search pitfall: faster early convergence (higher LR, 2× the trainable params) doesn't guarantee a better final result. | No — kept the official config. Logged in `experiments/checkpoints_optuna_tuned/`, chart in `images/experiment_optuna_search.png`. |

**Why more epochs alone is unlikely to help the face model:** `images/face_overfitting_proof.png` plots train vs. validation accuracy for all 20 trained epochs. Validation accuracy plateaus and oscillates between roughly 68–75% from epoch 8 onward and never exceeds its epoch-12 peak again, while training accuracy keeps climbing the entire time, reaching 96.3% by epoch 20 — a 22.6-point gap between the two by the end. That's the classic signature of overfitting once the last two transformer blocks have adapted to the training set. Note the honest limit of this evidence: training accuracy is still rising at epoch 20, so this run alone doesn't prove more epochs *can't* help — only that validation accuracy showed no further gains in the 20 epochs actually run, while the gap to training accuracy kept widening. The saved checkpoint uses epoch 12's weights (best val_acc), not epoch 20's, so this doesn't affect the deployed model either way.

**Why the audio model has more headroom:** unlike the face model, audio's train accuracy (43%) and val accuracy (42%) stayed close together the whole run — it's underfitting, not overfitting. More capacity or more data would help; more epochs on the current frozen setup would not (val accuracy was already flattening by epoch 14).

### 19.3b Multi-seed stability check — attempted, not completed

To check whether 78.36%/70.78% is a reliable number or a lucky random initialization, a second and third full run of the identical official config were started (`train.py` doesn't currently fix a random seed, so repeat runs naturally differ). **The second run was cut short by a GPU driver crash** ("GPU is lost, reboot the system to recover") partway through epoch 6/20 — a hardware/driver failure unrelated to any code in this project, that required stopping rather than risking the machine further. Only one full run of the official config exists. This is stated plainly rather than glossed over: the reported numbers are correct and reproducible, but their run-to-run variance has not been measured. Re-running the two remaining replicates (§19.5 command, on a confirmed-stable GPU) and reporting mean ± std is the single highest-value remaining piece of rigor for this project — see recommendation #1 below.

### 19.4 Recommendations for further improving accuracy

Ranked by expected effort-to-payoff, based on what the experiments above actually showed:

1. **Finish the multi-seed stability check (19.3b).** Cheapest remaining item and arguably the most important for thesis rigor: 2 more ~20-minute runs of the §19.5 command to turn a single 78.36% into a mean ± std.
2. **Audio: use the already-downloaded CREMA-D data via domain adaptation, not naive merging.** CREMA-D (7,442 clips, in `amal/audio_data_cremad/`) was combined with RAVDESS and tested (Section 19.3) — it made the RAVDESS benchmark *worse* (42.36% → 35.42%), not better, from domain shift between the corpora. Two more promising approaches, not yet tried: (a) pre-train on the combined set, then fine-tune a few epochs on RAVDESS-only to specialise back; or (b) treat CREMA-D as its own separate benchmark/model rather than merging corpora.
3. **Audio: a gentler unfreeze, not an all-or-nothing one.** Full unfreeze failed at 15 epochs; a middle ground — unfreeze only the last 2–4 Wav2Vec2 encoder layers (mirroring what `DINOFERModel` already does via `unfreeze_last_n_blocks`, which `AudioFERModel` currently lacks) — plus a longer warmup, is worth testing before ruling out unfreezing entirely.
4. **Face: augment or oversample the rarest classes instead of only reweighting the loss.** Class weighting alone bought +1.55pp macro F1; combining it with heavier augmentation specifically on Fear (281 images) and Disgust/Anger (~700–720 images each) — the three classes RAF-DB most under-represents — should compound with the loss-weighting gain rather than substitute for it.
5. **Face: try `vitb16` instead of `vits16`.** `models.py` already supports it (`--dino_variant vitb16`, 768-d embeddings, 86M params vs 21M). Worth one controlled run to see if the larger backbone's extra capacity helps once the last-N-blocks unfreezing strategy is already working, though it also raises fusion's input dimensionality and overfitting risk on the small datasets — validate before adopting.
6. **Fusion: replace same-class synthetic pairing with real paired data if any becomes available.** The current 89.90–93.51% fusion numbers are a ceiling estimate from pairing same-label face and audio samples, not evidence the model learned genuine cross-modal correlation — Layout A (`PairedFaceAudioDataset`, real aligned `fusion_labels.csv`) already exists in `train_multimodal.py` and would give a trustworthy number if real paired clips are ever collected.
7. **Wider Optuna sweep.** §19.3 ran 15 trials × 4 epochs and the winner didn't beat the official config at full length (a known short-budget-search pitfall) — a sweep with more epochs per trial would be more reliable, at higher GPU cost.

### 19.5 Reproducing this

```powershell
cd emotion_recognition_using_VST
.\venv\Scripts\python.exe src\train.py --model dino --dino_variant vits16 --epochs 20 --batch_size 32 --lr 1e-4 --class_weights --data_root . --output_dir checkpoints
.\venv\Scripts\python.exe src\train_audio.py --data_root audio_data --dataset ravdess --epochs 20 --batch_size 16 --lr 1e-4 --output_dir checkpoints
...
On Windows, pass `--num_workers 0` to the training scripts — DataLoader worker processes hung indefinitely in testing on this machine when `num_workers > 0` was combined with `pin_memory=True`; single-process loading was reliable and, for these dataset sizes, not meaningfully slower.
...
Train face model:   python src\train.py --class_weights  (recommended -- see Section 19)
